	...
	if request.GET.get("Sign_Up"):
		...
		if (F_Name != "" and L_Name != "" and Email != "" and P_Word_1 != "" and P_Word_1 == P_Word_2):
			if User.objects.filter(username = Email).exists():
				...
				New_User = User(first_name = F_Name, last_name = L_Name, email= Email, username = Email, password = P_Word_1)
				New_User.save()
...
def SignUp_Confirmation(request):
	...
	Packages = [["Gold ($480.00)", "G", 480], ["Silver ($300.00)", "S", 300], ["Bronze ($165.00)", "B", 165]]
